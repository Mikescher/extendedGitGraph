namespace ExtendedGitGraph2
{
    export function initHover()
    {
        for (const container of document.querySelectorAll('.extGitGraphContainer')) {
            ...
            const masterTip = container.querySelector('.svg-tip') as (HTMLElement|null);
            if (masterTip === null) continue;

            masterTip.style.opacity = '1';
            masterTip.style.display = 'none';

            const masterTipHeader  = masterTip.querySelector('.caption')! as HTMLElement;
            const masterTipContent = masterTip.querySelector('.date')!    as HTMLElement;
            const masterTipExtra   = masterTip.querySelector('.extra')!   as HTMLElement;

            const rects = Array.from(container.querySelectorAll(".egg_rect")) as HTMLElement[];
